...
<#
    Get-AdaptDomainForeignGroupMember.ps1 - Standalone Function
    Based on PowerView by Will Schroeder (@harmj0y)
    Original function: Get-AdaptDomainForeignGroupMember
...
# --- Main Function: Get-AdaptDomainForeignGroupMember ---
...
Author: Will Schroeder (@harmj0y)  
License: BSD 3-Clause  
Required Dependencies: Get-AdaptDomain, Get-AdaptDomainGroup  

.DESCRIPTION

Uses Get-AdaptDomainGroup to enumerate all groups for the current (or target) domain,
...
Get-AdaptDomainForeignGroupMember

Return all group members in the current domain where the group and member differ.

.EXAMPLE

Get-AdaptDomainForeignGroupMember -Domain dev.testlab.local
...
$SecPassword = ConvertTo-SecureString 'Password123!' -AsPlainText -Force
$Cred = New-Object System.Management.Automation.PSCredential('TESTLAB\dfm.a', $SecPassword)
Get-AdaptDomainForeignGroupMember -Domain dev.testlab.local -Server secondary.dev.testlab.local -Credential $Cred
