...
<#
    Get-AdaptNetRDPSession.ps1 - Fully Standalone Function
    Based on PowerView by Will Schroeder (@harmj0y)
    Original function: Get-AdaptNetRDPSession
...
# --- Main Function: Get-AdaptNetRDPSession ---
function Get-AdaptNetRDPSession {
<#
.SYNOPSIS
...
Get-AdaptNetRDPSession

Returns active RDP/terminal sessions on the local host.

.EXAMPLE

Get-AdaptNetRDPSession -ComputerName "sqlserver"

Returns active RDP/terminal sessions on the 'sqlserver' host.

.EXAMPLE

Get-AdaptDomainController | Get-AdaptNetRDPSession
...
$SecPassword = ConvertTo-SecureString 'Password123!' -AsPlainText -Force
$Cred = New-Object System.Management.Automation.PSCredential('TESTLAB\dfm.a', $SecPassword)
Get-AdaptNetRDPSession -ComputerName sqlserver -Credential $Cred
            ...
            if ($Handle -ne 0) {
                ...
                if (($Result -ne 0) -and ($Offset -gt 0)) {
                    ...
                    for ($i = 0; ($i -lt $pCount); $i++) {
                        ...
                        if ($Result2 -eq 0) {
                            Write-Verbose "[Get-AdaptNetRDPSession] Error: $(([ComponentModel.Win32Exception] $LastError2).Message)"
                        }
                        else {
                    ...
                    Write-Verbose "[Get-AdaptNetRDPSession] Error: $(([ComponentModel.Win32Exception] $LastError).Message)"
                }
                # close off the service handle
                $Null = $Wtsapi32::WTSCloseServer($Handle)
            }
            else {
                Write-Verbose "[Get-AdaptNetRDPSession] Error opening the Remote Desktop Session Host (RD Session Host) server for: $ComputerName"
            }
        }
    }
