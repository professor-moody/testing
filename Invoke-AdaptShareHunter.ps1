...
#>
    ...
    END {
        $startTime = Get-Date
        Write-Host ""
        Write-Host "     _    ____    _    ____ _____ " -ForegroundColor Cyan
        Write-Host "    / \  |  _ \  / \  |  _ \_   _|" -ForegroundColor Cyan
        Write-Host "   / _ \ | | | |/ _ \ | |_) || |  " -ForegroundColor Cyan
        Write-Host "  / ___ \| |_| / ___ \|  __/ | |  " -ForegroundColor Cyan
        Write-Host " /_/   \_\____/_/   \_\_|    |_|  " -ForegroundColor Cyan
        Write-Host "        Share Hunter v4           " -ForegroundColor Yellow
        Write-Host ""
        ...
        $shareScriptBlock = {
            ...
            # Fast share access check with timeout using async PowerShell
            function Test-ShareAccess {
                Param([String]$UNC, [Int]$TimeoutMs = 2000)
                try {
                    # Escape the UNC path properly to avoid injection issues
                    $escapedUNC = $UNC -replace "'", "''"
                    $ps = [PowerShell]::Create()
                    $null = $ps.AddScript("Test-Path -LiteralPath '$escapedUNC' -ErrorAction SilentlyContinue")
                    $handle = $ps.BeginInvoke()
                    $completed = $handle.AsyncWaitHandle.WaitOne($TimeoutMs)
                    if ($completed) {
                        $result = $ps.EndInvoke($handle)
                        $ps.Dispose()
                        # EndInvoke returns a collection - extract the actual boolean value
            ...
            # Note: net view won't show hidden shares (those ending in $)
            ...
            if ($netShares.Count -gt 0) {
                ...
                # DON'T return early - we still need to probe hidden $ shares
            }

            # Step 4: SMB probing for hidden shares (always run for $ shares)
            # This catches SCCM shares like SMSSIG$, SMS_DP$, etc. that don't show in net view
            foreach ($shareName in $CommonShares) {
                if ($ExcludedShares -notcontains $shareName) {
                    $unc = "\\$Computer\$shareName"
                    # Skip if we already found this share via WMI/net view
                    if ($results -contains $unc) { continue }
                    # Use longer timeout for hidden shares (3s instead of 2s)
                    $timeout = if ($shareName.EndsWith('$')) { 3000 } else { 2000 }
                    if (Test-ShareAccess -UNC $unc -TimeoutMs $timeout) {
                        $results += $unc
                    }
                }
            }
