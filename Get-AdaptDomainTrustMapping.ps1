...
<#
    Get-AdaptDomainTrustMapping.ps1 - Fully Standalone Function
    Based on PowerView by Will Schroeder (@harmj0y)
    Original function: Get-AdaptDomainTrustMapping
...
Author: Will Schroeder (@harmj0y)  
License: BSD 3-Clause  
Required Dependencies: Get-AdaptDomainComputer  

.DESCRIPTION

Returns the SID for the current domain or the specified domain by executing
Get-AdaptDomainComputer with the -LDAPFilter set to (userAccountControl:1.2.840.113556.1.4.803:=8192)
...
Get-AdaptDomainSID -Domain testlab.local

.EXAMPLE

$SecPassword = ConvertTo-SecureString 'Password123!' -AsPlainText -Force
$Cred = New-Object System.Management.Automation.PSCredential('TESTLAB\dfm.a', $SecPassword)
Get-AdaptDomainSID -Credential $Cred
...
# --- Get-AdaptDomainTrust (dependency) ---
function Get-AdaptDomainTrust {
<#
.SYNOPSIS
...
Author: Will Schroeder (@harmj0y)  
License: BSD 3-Clause  
Required Dependencies: Get-AdaptDomain, Get-DomainSearcher, Get-AdaptDomainSID, PSReflect  
...
$SecPassword = ConvertTo-SecureString 'Password123!' -AsPlainText -Force
$Cred = New-Object System.Management.Automation.PSCredential('TESTLAB\dfm.a', $SecPassword)
Get-AdaptDomainTrust -Domain "prod.testlab.local" -Server "PRIMARY.testlab.local" -Credential $Cred
...
Get-AdaptDomainTrust -API -Domain "prod.testlab.local"
    ...
    PROCESS {
        if ($PsCmdlet.ParameterSetName -ne 'API') {
            ...
            if ($Domain -and $Domain.Trim() -ne '') {
                ...
                if ($PSBoundParameters['Credential']) {
                    $SourceDomain = (Get-AdaptDomain -Credential $Credential).Name
                }
                else {
                    $SourceDomain = (Get-AdaptDomain).Name
                }
            }
        }
        ...
        if ($PsCmdlet.ParameterSetName -eq 'LDAP') {
            ...
            $SourceSID = Get-AdaptDomainSID @NetSearcherArguments
            ...
            if ($TrustSearcher) {
                ...
                if ($Results) {
                    try { $Results.dispose() }
                        ...
                        Write-Verbose "[Get-AdaptDomainTrust] Error disposing of the Results object: $_"
                    }
                }
        ...
        elseif ($PsCmdlet.ParameterSetName -eq 'API') {
            ...
            if (($Result -eq 0) -and ($Offset -gt 0)) {
                ...
                for ($i = 0; ($i -lt $DomainCount); $i++) {
                    ...
                    if ($Result -eq 0) {
                        Write-Verbose "[Get-AdaptDomainTrust] Error: $(([ComponentModel.Win32Exception] $LastError).Message)"
                    }
                    else {
                        $DomainTrust = New-Object PSObject
                        $DomainTrust | Add-Member Noteproperty 'SourceName' $SourceDomain
                ...
                Write-Verbose "[Get-AdaptDomainTrust] Error: $(([ComponentModel.Win32Exception] $Result).Message)"
            }
        }
        else {
            # if we're searching for domain trusts through .NET methods
            $FoundDomain = Get-AdaptDomain @NetSearcherArguments
            if ($FoundDomain) {
...
Get-AdaptForest -Forest external.domain

.EXAMPLE

$SecPassword = ConvertTo-SecureString 'Password123!' -AsPlainText -Force
$Cred = New-Object System.Management.Automation.PSCredential('TESTLAB\dfm.a', $SecPassword)
Get-AdaptForest -Credential $Cred
...
# --- Get-AdaptForestTrust (dependency) ---
...
Author: Will Schroeder (@harmj0y)  
License: BSD 3-Clause  
Required Dependencies: Get-AdaptForest  

.DESCRIPTION

This function will enumerate domain trust relationships for the current (or a remote)
forest using number of method using the .NET method GetAllTrustRelationships() on a
System.DirectoryServices.ActiveDirectory.Forest returned by Get-AdaptForest.
...
Get-AdaptForestTrust

Return current forest trusts.

.EXAMPLE

Get-AdaptForestTrust -Forest "external.local"
...
$SecPassword = ConvertTo-SecureString 'Password123!' -AsPlainText -Force
$Cred = New-Object System.Management.Automation.PSCredential('TESTLAB\dfm.a', $SecPassword)
Get-AdaptForestTrust -Forest "external.local" -Credential $Cred
...
# --- Main Function: Get-AdaptDomainTrustMapping ---
function Get-AdaptDomainTrustMapping {
<#
.SYNOPSIS
...
Author: Will Schroeder (@harmj0y)  
License: BSD 3-Clause  
Required Dependencies: Get-AdaptDomain, Get-AdaptDomainTrust, Get-AdaptForestTrust  
...
Get-AdaptDomainTrustMapping | Export-CSV -NoTypeInformation trusts.csv

Map all reachable domain trusts using .NET methods and output everything to a .csv file.

.EXAMPLE

Get-AdaptDomainTrustMapping -API | Export-CSV -NoTypeInformation trusts.csv

Map all reachable domain trusts using Win32 API calls and output everything to a .csv file.

.EXAMPLE

Get-AdaptDomainTrustMapping -NET | Export-CSV -NoTypeInformation trusts.csv
...
$SecPassword = ConvertTo-SecureString 'Password123!' -AsPlainText -Force
$Cred = New-Object System.Management.Automation.PSCredential('TESTLAB\dfm.a', $SecPassword)
Get-AdaptDomainTrustMapping -Server 'PRIMARY.testlab.local' | Export-CSV -NoTypeInformation trusts.csv
...
#>
    ...
    if ($PSBoundParameters['Credential']) {
        $CurrentDomain = (Get-AdaptDomain -Credential $Credential).Name
    }
    else {
        $CurrentDomain = (Get-AdaptDomain).Name
    }
    ...
    while($Domains.Count -ne 0) {
        ...
        if ($Domain -and ($Domain.Trim() -ne '') -and (-not $SeenDomains.ContainsKey($Domain))) {
            ...
            Write-Verbose "[Get-AdaptDomainTrustMapping] Enumerating trusts for domain: '$Domain'"
            ...
            try {
                ...
                $Trusts = Get-AdaptDomainTrust @DomainTrustArguments
                ...
                if ($PsCmdlet.ParameterSetName -eq 'NET') {
                    ...
                    $Trusts += Get-AdaptForestTrust @ForestTrustArguments
                }
                ...
                Write-Verbose "[Get-AdaptDomainTrustMapping] Error: $_"
            }
        }
    }
}
