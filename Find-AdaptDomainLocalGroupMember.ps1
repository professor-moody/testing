...
<#
    Find-AdaptDomainLocalGroupMember.ps1 - Standalone Function
    Based on PowerView by Will Schroeder (@harmj0y)
    Original function: Find-AdaptDomainLocalGroupMember
...
# --- Get-AdaptNetLocalGroup (dependency) ---
function Get-AdaptNetLocalGroup {
<#
.SYNOPSIS
...
Get-AdaptNetLocalGroup -Method Winnt
...
Get-AdaptNetLocalGroup -ComputerName primary.testlab.local
        ...
        ForEach ($Computer in $ComputerName) {
            if ($Method -eq 'API') {
                ...
                if (($Result -eq 0) -and ($Offset -gt 0)) {
                    ...
                    Write-Verbose "[Get-AdaptNetLocalGroup] Error: $(([ComponentModel.Win32Exception] $Result).Message)"
                }
            }
            else {
...
# --- Get-AdaptNetLocalGroupMember (dependency) ---
function Get-AdaptNetLocalGroupMember {
<#
.SYNOPSIS
...
Get-AdaptNetLocalGroupMember | ft
...
Get-AdaptNetLocalGroupMember -Method winnt | ft
...
Get-AdaptNetLocalGroup | Get-AdaptNetLocalGroupMember | ft
...
Get-AdaptNetLocalGroupMember -ComputerName primary.testlab.local | ft
            ...
            if ($Method -eq 'API') {
                ...
                if (($Result -eq 0) -and ($Offset -gt 0)) {
                    ...
                    for ($i = 0; ($i -lt $EntriesRead); $i++) {
                        ...
                        if ($Result2 -eq 0) {
                            Write-Verbose "[Get-AdaptNetLocalGroupMember] Error: $(([ComponentModel.Win32Exception] $LastError).Message)"
                        }
                        else {
                    ...
                    Write-Verbose "[Get-AdaptNetLocalGroupMember] Error: $(([ComponentModel.Win32Exception] $Result).Message)"
                }
            }
            else {
                # otherwise we're using the WinNT service provider
                try {
                    ...
                    Write-Verbose "[Get-AdaptNetLocalGroupMember] Error for $Computer : $_"
                }
            }
        }
    }
...
# --- Main Function: Find-AdaptDomainLocalGroupMember ---
function Find-AdaptDomainLocalGroupMember {
<#
.SYNOPSIS
...
Author: Will Schroeder (@harmj0y)  
License: BSD 3-Clause  
Required Dependencies: Get-AdaptDomainComputer, Invoke-AdaptUserImpersonation, Invoke-AdaptRevertToSelf, Get-AdaptNetLocalGroupMember, New-AdaptThreadedFunction  

.DESCRIPTION

This function enumerates all machines on the current (or specified) domain
using Get-AdaptDomainComputer, and enumerates the members of the specified local
group (default of Administrators) for each machine using Get-AdaptNetLocalGroupMember.
...
Find-AdaptDomainLocalGroupMember

Enumerates the local group memberships for all reachable machines in the current domain.

.EXAMPLE

Find-AdaptDomainLocalGroupMember -Domain dev.testlab.local
...
$SecPassword = ConvertTo-SecureString 'Password123!' -AsPlainText -Force
$Cred = New-Object System.Management.Automation.PSCredential('TESTLAB\dfm.a', $SecPassword)
Find-AdaptDomainLocalGroupMember -Domain testlab.local -Credential $Cred
...
#>
    ...
    BEGIN {
        ...
        if ($PSBoundParameters['ComputerName']) {
            ...
            Write-Verbose '[Find-AdaptDomainLocalGroupMember] Querying computers in the domain'
            $TargetComputers = Get-AdaptDomainComputer @ComputerSearcherArguments | Select-Object -ExpandProperty dnshostname
        }
        Write-Verbose "[Find-AdaptDomainLocalGroupMember] TargetComputers length: $($TargetComputers.Length)"
        if ($TargetComputers.Length -eq 0) {
            throw '[Find-AdaptDomainLocalGroupMember] No hosts found to enumerate'
        }
        ...
        $HostEnumBlock = {
            ...
            ForEach ($TargetComputer in $ComputerName) {
                ...
                if ($Up) {
                    ...
                    Get-AdaptNetLocalGroupMember @NetLocalGroupMemberArguments
                }
            }
    ...
    PROCESS {
        ...
        if ($PSBoundParameters['Delay'] -or $PSBoundParameters['StopOnSuccess']) {
            ...
            Write-Verbose "[Find-AdaptDomainLocalGroupMember] Total number of hosts: $($TargetComputers.count)"
            Write-Verbose "[Find-AdaptDomainLocalGroupMember] Delay: $Delay, Jitter: $Jitter"
            $Counter = 0
            ...
            ForEach ($TargetComputer in $TargetComputers) {
                ...
                Write-Verbose "[Find-AdaptDomainLocalGroupMember] Enumerating server $TargetComputer ($Counter of $($TargetComputers.count))"
                Invoke-Command -ScriptBlock $HostEnumBlock -ArgumentList $TargetComputer, $GroupName, $Method, $LogonToken
            }
        }
        else {
            Write-Verbose "[Find-AdaptDomainLocalGroupMember] Using threading with threads: $Threads"
