...
<#
    Get-AdaptDomainPolicyData.ps1 - Fully Standalone Function
    Based on PowerView by Will Schroeder (@harmj0y)
    Original function: Get-AdaptDomainPolicyData
...
# --- Main Function: Get-AdaptDomainPolicyData ---
function Get-AdaptDomainPolicyData {
<#
.SYNOPSIS
...
Author: Will Schroeder (@harmj0y)  
License: BSD 3-Clause  
Required Dependencies: Get-AdaptDomainGPO, Get-AdaptGptTmpl, ConvertFrom-SID  

.DESCRIPTION

Returns the default domain policy or the domain controller policy for the current
domain or a specified domain/domain controller using Get-AdaptDomainGPO.
...
$SecPassword = ConvertTo-SecureString 'Password123!' -AsPlainText -Force
$Cred = New-Object System.Management.Automation.PSCredential('TESTLAB\dfm.a', $SecPassword)
Get-AdaptDomainPolicyData -Credential $Cred
...
#>
    ...
    PROCESS {
        ...
        $GPOResults = Get-AdaptDomainGPO @SearcherArguments
        ...
        ForEach ($GPO in $GPOResults) {
            ...
            Get-AdaptGptTmpl @ParseArgs | ForEach-Object {
