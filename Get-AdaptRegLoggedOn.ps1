...
<#
    Get-AdaptRegLoggedOn.ps1 - Standalone Function
    Based on PowerView by Will Schroeder (@harmj0y)
    Original function: Get-AdaptRegLoggedOn
...
# --- Main Function: Get-AdaptRegLoggedOn ---
function Get-AdaptRegLoggedOn {
<#
.SYNOPSIS
...
License: BSD 3-Clause  
Required Dependencies: Invoke-AdaptUserImpersonation, Invoke-AdaptRevertToSelf, ConvertFrom-SID  
...
using the NetWkstaUserEnum API (Get-AdaptNetLoggedon) of less user privileges
...
Get-AdaptRegLoggedOn

Returns users actively logged onto the local host.

.EXAMPLE

Get-AdaptRegLoggedOn -ComputerName sqlserver

Returns users actively logged onto the 'sqlserver' host.

.EXAMPLE

Get-AdaptDomainController | Get-AdaptRegLoggedOn
...
$SecPassword = ConvertTo-SecureString 'Password123!' -AsPlainText -Force
$Cred = New-Object System.Management.Automation.PSCredential('TESTLAB\dfm.a', $SecPassword)
Get-AdaptRegLoggedOn -ComputerName sqlserver -Credential $Cred
    ...
    PROCESS {
        ForEach ($Computer in $ComputerName) {
            try {
                ...
                Write-Verbose "[Get-AdaptRegLoggedOn] Error opening remote registry on '$ComputerName' : $_"
            }
        }
    }
