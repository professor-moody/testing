...
<#
    Find-AdaptLocalAdminAccess.ps1 - Standalone Function
    Based on PowerView by Will Schroeder (@harmj0y)
    Original function: Find-AdaptLocalAdminAccess
...
# --- Main Function: Find-AdaptLocalAdminAccess ---
function Find-AdaptLocalAdminAccess {
<#
.SYNOPSIS
...
Author: Will Schroeder (@harmj0y)  
License: BSD 3-Clause  
Required Dependencies: Get-AdaptDomainComputer, Invoke-AdaptUserImpersonation, Invoke-AdaptRevertToSelf, Test-AdaptAdminAccess, New-AdaptThreadedFunction  

.DESCRIPTION

This function enumerates all machines on the current (or specified) domain
using Get-AdaptDomainComputer, and for each computer it checks if the current user
has local administrator access using Test-AdaptAdminAccess. If -Credential is passed,
then Invoke-AdaptUserImpersonation is used to impersonate the specified user
before enumeration, reverting after with Invoke-AdaptRevertToSelf.
...
Find-AdaptLocalAdminAccess

Finds machines in the current domain the current user has admin access to.

.EXAMPLE

Find-AdaptLocalAdminAccess -Domain dev.testlab.local
...
$SecPassword = ConvertTo-SecureString 'Password123!' -AsPlainText -Force
$Cred = New-Object System.Management.Automation.PSCredential('TESTLAB\dfm.a', $SecPassword)
Find-AdaptLocalAdminAccess -Domain testlab.local -Credential $Cred
...
#>
    ...
    BEGIN {
        ...
        if ($PSBoundParameters['ComputerName']) {
            ...
            Write-Verbose '[Find-AdaptLocalAdminAccess] Querying computers in the domain'
            $TargetComputers = Get-AdaptDomainComputer @ComputerSearcherArguments | Select-Object -ExpandProperty dnshostname
        }
        Write-Verbose "[Find-AdaptLocalAdminAccess] TargetComputers length: $($TargetComputers.Length)"
        if ($TargetComputers.Length -eq 0) {
            throw '[Find-AdaptLocalAdminAccess] No hosts found to enumerate'
        }
        ...
        $HostEnumBlock = {
            ...
            ForEach ($TargetComputer in $ComputerName) {
                ...
                if ($Up) {
                    ...
                    $Access = Test-AdaptAdminAccess -ComputerName $TargetComputer
                    if ($Access.IsAdmin) {
                        $TargetComputer
                    }
                }
    ...
    PROCESS {
        ...
        if ($PSBoundParameters['Delay'] -or $PSBoundParameters['StopOnSuccess']) {
            ...
            Write-Verbose "[Find-AdaptLocalAdminAccess] Total number of hosts: $($TargetComputers.count)"
            Write-Verbose "[Find-AdaptLocalAdminAccess] Delay: $Delay, Jitter: $Jitter"
            $Counter = 0
            ...
            ForEach ($TargetComputer in $TargetComputers) {
                ...
                Write-Verbose "[Find-AdaptLocalAdminAccess] Enumerating server $TargetComputer ($Counter of $($TargetComputers.count))"
                Invoke-Command -ScriptBlock $HostEnumBlock -ArgumentList $TargetComputer, $LogonToken
            }
        }
        else {
            Write-Verbose "[Find-AdaptLocalAdminAccess] Using threading with threads: $Threads"
