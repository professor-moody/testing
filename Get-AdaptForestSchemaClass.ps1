...
<#
    Get-AdaptForestSchemaClass.ps1 - Fully Standalone Function
    Based on PowerView by Will Schroeder (@harmj0y)
    Original function: Get-AdaptForestSchemaClass
...
# --- Main Function: Get-AdaptForestSchemaClass ---
...
Uses Get-AdaptForest to retrieve the current (or specified) forest. By default,
...
Get-AdaptForestSchemaClass

Returns all domain schema classes for the current forest.

.EXAMPLE

Get-AdaptForestSchemaClass -Forest dev.testlab.local

Returns all domain schema classes for the external.local forest.

.EXAMPLE

Get-AdaptForestSchemaClass -ClassName user -Forest external.local
...
$SecPassword = ConvertTo-SecureString 'Password123!' -AsPlainText -Force
$Cred = New-Object System.Management.Automation.PSCredential('TESTLAB\dfm.a', $SecPassword)
Get-AdaptForestSchemaClass -ClassName user -Forest external.local -Credential $Cred
