...
<#
    Get-AdaptDomainForeignUser.ps1 - Fully Standalone Function
    Based on PowerView by Will Schroeder (@harmj0y)
    Original function: Get-AdaptDomainForeignUser
...
# --- Main Function: Get-AdaptDomainForeignUser ---
...
Author: Will Schroeder (@harmj0y)  
License: BSD 3-Clause  
Required Dependencies: Get-AdaptDomain, Get-AdaptDomainUser  

.DESCRIPTION

Uses Get-AdaptDomainUser to enumerate all users for the current (or target) domain,
...
Get-AdaptDomainForeignUser -Domain dev.testlab.local
...
$SecPassword = ConvertTo-SecureString 'Password123!' -AsPlainText -Force
$Cred = New-Object System.Management.Automation.PSCredential('TESTLAB\dfm.a', $SecPassword)
Get-AdaptDomainForeignUser -Domain dev.testlab.local -Server secondary.dev.testlab.local -Credential $Cred
